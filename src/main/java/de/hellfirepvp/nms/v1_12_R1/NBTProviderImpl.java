...
public class NBTProviderImpl implements NBTProvider
{
    ...
    public ItemStack loadStack(final WrappedNBTTagCompound savedStack) {
        ...
        NBTTagCompound out = (NBTTagCompound)savedStack.getRawNMSTagCompound();
        out = MinecraftServer.getServer().dataConverterManager.a((DataConverterType)DataConverterTypes.ITEM_INSTANCE, out);
        return CraftItemStack.asBukkitCopy(new net.minecraft.server.v1_12_R1.ItemStack(out));
    }
    ...
    public static class TagListImpl implements WrappedNBTTagList
    {
        ...
        public Object getElementAtIndex(final int index) {
            final NBTBase element = this.parentList.i(index);
    ...
    public static class ForIntIterator implements NullableIndexedElementIterator<Object>
    {
        ...
        public Object next() {
            ++this.entryPointer;
            this.removeCalledThisIteration = false;
            final NBTBase element = this.list.i(this.entryPointer);
            final Object value = NBTProviderImpl.extractValue(element);
            if (value != null) {
                return value;
            }
